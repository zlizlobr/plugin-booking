 ...
 * Handles the automation conditions for hospital status.
 * Integrates with FluentCRM to assess hospital status in automation funnels.
 */
class hospitalStatusCondition
{
     ...
     * Constructor
     * 
     * Sets up the condition filters for hospital status assessment.
     */
     ...
     * @return array
     */
    public function add_automation_conditions($children, $funnel)
    {
        // Only available for hospital karma trigger
        if ($funnel->trigger_name !== 'nbh_hospital_status') {
            return $children;
        }
        ...
        $children[] = [
            ...
            'options' => [
                'empty_hospital'      => __('Empty Hospital', 'nbh'),
                'unequipped_hospital' => __('Unequipped Hospital', 'nbh'),
                'completed_hospital'  => __('Completed Hospital', 'nbh')
            ]
        ];
    ...
    public function assess_automation_conditions($result, $conditions, $subscriber)
    {
        ...
        // Get current hospital status from ACF field
        $current_status = get_field(\NBH\Includes\FluentCRMIntegration::ACF_STATUS_NAME, $hospital_id);

        // Prepare input values for condition assessment
        $input_values = [
            'HospitalStatus' => $current_status
        ];
